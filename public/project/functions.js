var functions_d6ad677b_427a_4623_b50f_a445a3b0ef8a =
{
    "events": {
        ...
        "win": function (reason, norank, noexit) {
            ...
            if (core.status.hero.atk >= 1000 || core.status.hero.def >= 1000) flags.cheat += 50;
            if (flags.cheat >= 30) {
                while (true) {
            ...
            if (reason == '智慧之始')
                core.status.hero.hp += core.itemCount('yellowKey') * 5000 + core.itemCount('blueKey') * 15000;
        ...
        "afterBattle": function (enemyId, x, y) {
            ...
            if (core.hasSpecial(special, 14) && flags.hard == 2) {
                ...
                core.addFlag("inte_" + floorId, Math.ceil(core.status.hero.mdef / 10 * 0.3) * 10);
                core.status.hero.mdef -= Math.ceil(core.status.hero.mdef / 10 * 0.3) * 10;
            }
            ...
            if (core.status.hero.loc.direction == 'up' || core.status.hero.loc.direction == 'down')
        ...
        "afterGetItem": function (itemId, x, y, isGentleClick) {
            ...
            if ((itemId.endsWith('Potion') || itemId == "I482" || itemId == "I484" ||
                itemId == "I487" || itemId == "I491") && core.material.items[itemId].cls == 'items')
                core.playSound('回血');
            else
                core.playSound('获得道具');
            ...
            if (core.hasFlag("spring")) {
                ...
                if (itemId.endsWith("Potion") || itemId == "I482" || itemId == "I484" || itemId == "I487" || itemId == "I491") {
    ...
    "enemys": {
        "getSpecials": function () {
            ...
            return [
                [1, "致命一击", function (enemy) { return "怪物每5回合触发一次强力攻击，造成" + (enemy.crit || 100) + "%的伤害" }, "#ffcc33"],
                ...
                [7, "饥渴", function (enemy) { return "战斗前，怪物偷取勇士" + (enemy.hungry || 0) + "%加在自己身上（勇士攻击也会降低）"; }, "#b30000"],
                [8, "抱团", function (enemy) { return "怪物周围5×5范围内每有一个拥有该属性的怪物，该怪物攻防就增加" + (enemy.together || 0) + "%（线性叠加）"; }, "#ffaa44", 1],
                [9, "绝对防御", function () { return "怪物的奇特护甲可以让勇士的额外攻击变为正常攻击（相当于勇士的攻击变为基础攻击+额外攻击）"; }, "#80eed6"],
                [10, "勇气之刃", function (enemy) { return "第一回合造成" + (enemy.courage || 100) + "%的伤害，之后正常"; }, "#b0c0dd"],
                [11, "勇气冲锋", function (enemy) { return "怪物首先发动冲锋，造成" + (enemy.charge || 100) + "%的伤害，并眩晕勇士5回合"; }, "#ff00d2"],
                [12, "追猎", "当勇士移动到该怪物的水平或竖直方向上时，怪物向勇士移动一格", "#99ee88", 2],
                [13, "魔攻", "怪物攻击无视勇士防御", "#bbb0ff"],
                [14, "智慧之源", "困难难度下（简单难度没有效果），战斗后，怪物会吸取勇士30%的智慧（勇士智慧向下取整至整十）加在本层的拥有该属性的怪物攻击上", "#bbeef0"],
                [15, "突刺", function (enemy) {
                    return "经过怪物周围" + (enemy.zoneSquare ? "九宫格" : "十字") + "范围内" + (enemy.range || 1) + "格时怪物会攻击勇士，造成" +
                        core.formatBigNumber(Math.max(((enemy.value || 0) - core.getRealStatusOrDefault(null, 'def')), 0)) + "点伤害";
                }, "#c677dd"],
                ...
                [24, "射击", function () { return "经过怪物同行或同列的可视范围内时受到一次普通攻击的伤害"; }, "#dda0dd"],
        ...
        "getEnemyInfo": function (enemy, hero, x, y, floorId) {
            ...
            if (core.status.checkBlock.needCache) {
                ...
                var usedEnemyIds = {};
                // 检查光环和支援的缓存
                var index = x != null && y != null ? (x + "," + y) : ("floor" + enemy.id);
                ...
                if (!cache) {
                    ...
                    core.status.maps[floorId].blocks.forEach(function (block) {
                        if (!block.disable) {
                            ...
                            // 抱团
                            if (enemy && core.hasSpecial(mon_special, 8) && core.hasSpecial(enemy.special, 8) && !(dx == 0 && dy == 0) && dx < 3 && dy < 3) {
        ...
        "getDamageInfo": function (enemy, hero, x, y, floorId) {
            ...
            // 魔攻：战斗伤害就是怪物攻击力
            if (core.hasSpecial(mon_special, 2) || core.hasSpecial(mon_special, 13)) per_damage = mon_atk;
            // 战斗伤害不能为负值
            ...
            if (!core.hasSpecial(mon_special, 9)) {
                ...
                if (hero_per_damage > 0)
                    hero_per_damage += hero_extraAtk;
            }
            if (core.hasSpecial(mon_special, 9)) {
                var hero_per_damage = Math.max(hero_atk + hero_extraAtk - mon_def, 0);
            }
            ...
            switch (keyCode) {
                ...
                case 65: // A：读取自动存档（回退）
                    if (core.status.floorId != 'tower6') {
                        ...
                        core.myconfirm('确定要读取自动存档吗？读取后无法通过按W回退至现在', function () {
                            core.doSL("autoSave", "load");
                        }, function () { return; });
                    }
                    break;
                case 87: // W：撤销回退
                    if (core.status.floorId != 'tower6') {
                        ...
                        core.myconfirm('确定要撤销回退吗？撤销后后无法通过按A回退至现在', function () {
                            core.doSL("autoSave", "reload");
                        }, function () { return; });
                    }
                    break;
                ...
                case 49: // 1: 断灭之刃
                    ...
                    break;
                case 50: // 快捷键2: 跳跃技能 || 破
                    if (!flags.chase && !core.status.floorId.startsWith("tower") && flags.skill2) {
                        core.jumpSkill();
                ...
                case 53: // 5：读取自动存档（回退），方便手机版操作
                    if (core.status.floorId != 'tower6') {
                        ...
                        core.myconfirm('确定要读取自动存档吗？读取后无法通过按6回退至现在', function () {
                            core.doSL("autoSave", "load");
                        }, function () { return; });
                    }
                    break;
                case 54: // 6：撤销回退，方便手机版操作
                    if (core.status.floorId != 'tower6') {
                        ...
                        core.myconfirm('确定要撤销回退吗？撤销后后无法通过按5回退至现在', function () {
                            core.doSL("autoSave", "reload");
                        }, function () { return; });
                    }
                    break;
    ...
    "control": {
        "saveData": function () {
            ...
            var data = {
                ...
                "tower6": core.saveLoopMap("tower6", ['tower1', 'tower2', 'tower4', 'tower5']),
            };
                ...
                if (enemy && core.hasSpecial(enemy.special, 15) && !core.hasFlag('no_zone')) {
                    ...
                    for (var dx = -range; dx <= range; dx++) {
                        for (var dy = -range; dy <= range; dy++) {
                            ...
                            damage[currloc] = Math.max((damage[currloc] || 0) + (enemy.value || 0) - core.getRealStatusOrDefault(null, 'def'), 0);
                ...
                if (enemy && core.hasSpecial(enemy.special, 24)) {
                    ...
                    for (var nx = 0; nx < width; nx++) {
                        ...
                        for (var mx = nx; mx != x; mx > x ? mx-- : mx++) {
                            if (core.getBlockCls(mx, y, floorId) == "enemys" || core.getBlockCls(mx, y, floorId) == "enemy48") continue;
                            if (core.noPass(mx, y, floorId) && core.getBlockNumber(mx, y, floorId) != 141 && core.getBlockNumber(mx, y, floorId) != 151) {
                                beyondVisual = true;
                                break;
                            }
                        }
                        ...
                        if (nx != x && !(core.getBlockCls(nx, y, floorId) == "enemys" || core.getBlockCls(nx, y, floorId) == "enemy48")) {
                            damage[currloc] = (damage[currloc] || 0) + Math.max((enemy.atk || 0) - core.getRealStatusOrDefault(null, 'def'), 0);
                            type[currloc] = type[currloc] || {};
                    ...
                    for (var ny = 0; ny < height; ny++) {
                        ...
                        for (var my = ny; my != y; my > y ? my-- : my++) {
                            if (core.getBlockCls(x, my, floorId) == "enemys" || core.getBlockCls(x, my, floorId) == "enemy48") continue;
                            if (core.noPass(x, my, floorId) && core.getBlockNumber(x, my, floorId) != 141 && core.getBlockNumber(x, my, floorId) != 151) {
                                beyondVisual = true;
                                break;
                            }
                        }
                        ...
                        if (ny != y && !(core.getBlockCls(x, ny, floorId) == "enemys" || core.getBlockCls(x, ny, floorId) == "enemy48")) {
                            damage[currloc] = (damage[currloc] || 0) + Math.max((enemy.atk || 0) - core.getRealStatusOrDefault(null, 'def'), 0);
        ...
        "moveOneStep": function (callback) {
            ...
            if (core.status.checkBlock.haveHunt) {
                ...
                core.status.maps[core.status.floorId].blocks.forEach(function (block) {
                    ...
                    if (enemy && core.hasSpecial(enemy.special, 12)) {
                        ...
                        var dx = Math.abs(x - nx),
                            dy = Math.abs(y - ny);
                        if (x == block.x) {
                            if (y > block.y && !core.noPass(block.x, block.y + 1) && core.getBlockCls(block.x, block.y + 1) != 'items') {
                                dy--;
                                ny++;
                                core.insertAction([
                                    { "type": "move", "loc": [block.x, block.y], "time": 200, "keep": true, "steps": ["down:1"] },
                                    {
                                        "type": "if",
                                        "condition": dy + "<=1",
                                        "true": [
                            ...
                            if (y < block.y && !core.noPass(block.x, block.y - 1) && core.getBlockCls(block.x, block.y - 1) != 'items') {
                                ...
                                ny--;
                                core.insertAction([
                                    { "type": "move", "loc": [block.x, block.y], "time": 200, "keep": true, "steps": ["up:1"] },
                                    {
                                        "type": "if",
                                        "condition": dy + "<=1",
                                        "true": [
                            ...
                            if (x > block.x && !core.noPass(block.x + 1, block.y) && core.getBlockCls(block.x + 1, block.y) != 'items') {
                                dx--;
                                nx++;
                                core.insertAction([
                                    { "type": "move", "loc": [block.x, block.y], "time": 200, "keep": true, "steps": ["right:1"] },
                                    {
                                        "type": "if",
                                        "condition": dx + "<=1",
                                        "true": [
                            ...
                            if (x < block.x && !core.noPass(block.x - 1, block.y) && core.getBlockCls(block.x - 1, block.y) != 'items') {
                                ...
                                nx--;
                                core.insertAction([
                                    { "type": "move", "loc": [block.x, block.y], "time": 200, "keep": true, "steps": ["left:1"] },
                                    {
                                        "type": "if",
                                        "condition": dx + "<=1",
                                        "true": [
    ...
    "ui": {
        ...
        "drawStatusBar": function () {
            ...
            if (!core.domStyle.isVertical) {
                ...
                if (flags.blade && flags.bladeOn)
                    core.fillBoldText(ctx, "断灭之刃", 145, 47, "#ffffff", "#cc3333", "16px normal");
                core.setTextAlign(ctx, "left");
                // 楼层名+等级名
                core.fillBoldText(ctx, (core.status.thisMap || {}).title || "", 10, 22, "#ffffff", "#000000", "20px normal");
                ...
                ctx.stroke();
                ctx.beginPath();
                core.fillBoldText(ctx, core.getLvName(), 10, 47, "#ffffff", "#000000", "20px normal");
                // 各个属性
                core.drawImage(ctx, "hp.png", 5, 55, 32, 32);
                core.fillBoldText(ctx, core.formatBigNumber(hp), 42, 80, "#ffffff", "#000000", "27px normal");
                // 生命回复
                core.fillBoldText(ctx, "+" + core.formatBigNumber(recovery) + "/t", 100, 95, "#ffffff", "#00cc00", "16px normal");
                // 治愈之泉
                if (core.hasFlag("spring"))
                    core.fillBoldText(ctx, "剩余" + (50 - core.getFlag("springCount", 0)), 90, 60, "#ffffff", "#00cc00", "16px normal");
                // 额外攻击
                core.fillBoldText(ctx, "+" + core.formatBigNumber(extraAtk), 100, 127, "#ffffff", "#cc6666", "16px normal");
                // 其他
                core.drawImage(ctx, "atk.png", 5, 87, 32, 32);
                core.fillBoldText(ctx, core.formatBigNumber(atk), 42, 112, "#ffffff", "#000000", "27px normal");
                core.drawImage(ctx, "def.png", 5, 119, 32, 32);
                core.fillBoldText(ctx, core.formatBigNumber(def), 42, 144, "#ffffff", "#000000", "27px normal");
                core.drawImage(ctx, "IQ.png", 5, 151, 32, 32);
                core.fillBoldText(ctx, core.formatBigNumber(inte), 42, 176, "#ffffff", "#000000", "27px normal");
                core.drawImage(ctx, "money.png", 5, 183, 32, 32);
                core.fillBoldText(ctx, core.formatBigNumber(core.getRealStatusOrDefault(null, "money")), 42, 208, "#ffffff", "#000000", "27px normal");
                core.drawImage(ctx, "exp.png", 5, 215, 32, 32);
                core.fillBoldText(ctx, core.formatBigNumber(core.getNextLvUpNeed() || 0), 42, 240, "#ffffff", "#000000", "27px normal");
                // 绘制三色钥匙
                core.fillBoldText(ctx, core.setTwoDigits(core.itemCount('yellowKey')), 15, 270, "#ffccaa", "#000000", "bold 27px normal");
                core.fillBoldText(ctx, core.setTwoDigits(core.itemCount('blueKey')), 55, 270, "#aaaadd", "#000000", "bold 27px normal");
                core.fillBoldText(ctx, core.setTwoDigits(core.itemCount('redKey')), 95, 270, "#ff8888", "#000000", "bold 27px normal");
                // 技能树 和 技能查看
                if (core.getFlag("chapter", 0) > 0) {
                    ...
                    ctx.shadowOffsetY = 2;
                    ctx.shadowBlur = 3;
                    core.fillBoldText(ctx, "技能树", 75, 340, "#ffffff", "#000000", "20px normal");
                    core.fillBoldText(ctx, "查看技能", 75, 300, "#ffffff", "#000000", "20px normal");
                    core.setTextAlign(ctx, "left");
                }
            } else {
                // 竖屏模式
                ...
                core.fillBoldText(ctx, core.formatBigNumber(hp), 42, 29, "#ffffff", "#000000", "27px normal");
                // 治愈之泉
                if (core.hasFlag("spring"))
                    core.fillBoldText(ctx, "剩余" + (50 - core.getFlag("springCount", 0)), 220, 41, "#ffffff", "#00cc00", "16px normal");
                // 生命回复
                core.fillBoldText(ctx, "+" + core.formatBigNumber(recovery) + "/t", 100, 41, "#ffffff", "#00cc00", "16px normal");
                // 额外攻击
                core.fillBoldText(ctx, "+" + core.formatBigNumber(extraAtk), 100, 76, "#ffffff", "#cc6666", "16px normal");
                core.drawImage(ctx, "atk.png", 5, 36, 32, 32);
                core.fillBoldText(ctx, core.formatBigNumber(atk), 42, 61, "#ffffff", "#000000", "27px normal");
                core.drawImage(ctx, "def.png", 5, 68, 32, 32);
                core.fillBoldText(ctx, core.formatBigNumber(def), 42, 93, "#ffffff", "#000000", "27px normal");
                core.drawImage(ctx, "IQ.png", 145, 4, 32, 32);
                core.fillBoldText(ctx, core.formatBigNumber(inte), 182, 29, "#ffffff", "#000000", "27px normal");
                core.drawImage(ctx, "money.png", 145, 36, 32, 32);
                core.fillBoldText(ctx, core.formatBigNumber(core.getRealStatusOrDefault(null, "money")), 182, 61, "#ffffff", "#000000", "27px normal");
                core.drawImage(ctx, "exp.png", 145, 68, 32, 32);
                core.fillBoldText(ctx, core.formatBigNumber(core.getNextLvUpNeed() || 0), 182, 93, "#ffffff", "#000000", "27px normal");
                // 楼层名 + 等级名 + 技能名
                core.setTextAlign(ctx, "right");
                core.fillBoldText(ctx, (core.status.thisMap || {}).title || "", 385, 52, "#ffffff", "#000000", "20px normal");
                core.drawLine(ctx, 282, 57, 390, 57, "#ffffff", 2);
                core.drawLine(ctx, 312, 82, 390, 82, "#ffffff", 1);
                core.fillBoldText(ctx, core.getLvName(), 385, 77, "#ffffff", "#000000", "20px normal");
                if (flags.blade && flags.bladeOn)
                    core.fillBoldText(ctx, "断灭之刃", 385, 97, "#ffffff", "#cc3333", "16px normal");
                core.setTextAlign(ctx, "left");
                ...
                if (core.getFlag("chapter", 0) > 0) {
                    ...
                    ctx.shadowOffsetY = 0;
                    ctx.shadowBlur = 5;
                    core.drawImage(ctx, 'botton.png', 0, 0, 190, 60, 375, 40, 85, 30, -Math.PI / 2);
                    core.drawImage(ctx, 'botton.png', 0, 0, 190, 60, 415, 40, 85, 30, -Math.PI / 2);
                    ...
                    ctx.shadowBlur = 3;
                    core.setTextAlign(ctx, "center");
                    core.fillBoldText(ctx, "技", 415, 42, "#ffffff", "#000000", "20px normal");
                    core.fillBoldText(ctx, "能", 415, 62, "#ffffff", "#000000", "20px normal");
                    core.fillBoldText(ctx, "树", 415, 82, "#ffffff", "#000000", "20px normal");
                    core.fillBoldText(ctx, "查", 455, 30, "#ffffff", "#000000", "20px normal");
                    core.fillBoldText(ctx, "看", 455, 50, "#ffffff", "#000000", "20px normal");
                    core.fillBoldText(ctx, "技", 455, 70, "#ffffff", "#000000", "20px normal");
                    core.fillBoldText(ctx, "能", 455, 90, "#ffffff", "#000000", "20px normal");
                    core.setTextAlign(ctx, "left");
                }

                // 绘制三色钥匙
                core.fillBoldText(ctx, core.setTwoDigits(core.itemCount('yellowKey')), 282, 29, '#FFCCAA', '#000000', "bold 27px normal");
                core.fillBoldText(ctx, core.setTwoDigits(core.itemCount('blueKey')), 317, 29, '#AAAADD', '#000000', "bold 27px normal");
                core.fillBoldText(ctx, core.setTwoDigits(core.itemCount('redKey')), 352, 29, '#FF8888', '#000000', "bold 27px normal");
            }
        },
